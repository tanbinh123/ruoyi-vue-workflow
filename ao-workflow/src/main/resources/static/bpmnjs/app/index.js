...
$(function () {
    ...
    $('.item').show()
    if(!param) {
        console.log("param not exists.");
    } else if (param.type === 'addBpmn') {
        tools.createDiagram(diagramXML, bpmnModeler, container);
    } else if (param.type === 'lookBpmn') { //编辑bpmn
        $('.item').hide()
        $('.download').show()
        const Id = param.deploymentFileUUID || 'a9d204aa-5295-11eb-b811-2e4cd71b5f19'
        const Name = param.deploymentName || 'diagram-form.bpmn'
        ...
        if (instanceId) {
            ...
            $.ajax({
                url: publicurl + 'activiti7/historic/getHighLine',
                type: 'GET',
                data: param1,
                dataType: 'json',
                success: function (result) {
                    var ColorJson = tools.getByColor(result)
                    $.ajax({
                        url: publicurl + 'activiti7/processDef/BPMNXMLResource',
                        type: 'GET',
                        data: param,
                        dataType: 'text',
                        success: function (result) {
    ...
    // 点击新增
    $('#js-download-diagram').on("click", function (event) {
    ...
    // 点击下载
    $("#downloadBpmn").on("click", function (event) {
